...
const createCategory = async (categoryData, ownerId) => {
    const { restaurantId, name } = categoryData;

    const normalizedName = name.trim().toLowerCase(); //this is done so that you can check that the new category you are creating does not already exist
    ...
    if (
        restaurant.owner.toString() !== ownerId.toString() //every restaurant has an owner and so you are checking if the user who is adding category is the owner
    ) {
        throw new Error(
...
const getCategoriesByRestaurant = async (restaurantId) => {
    const restaurant = await Restaurant.findById(restaurantId); //first check if the restaurant exists
...
const updateCategory = async (
    ...
    if (updateData.name) {
        const normalizedName = updateData.name.trim().toLowerCase(); //we store normalized names so thats its easy for comparison to check if category exists instead of fetching the name first and then converting to lowercase and checking
