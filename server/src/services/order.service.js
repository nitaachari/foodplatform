...
const ORDER_STATUS_TRANSITIONS = {
    //this refers to for every status what is the transition refers to the order lifecycle and from one state what are the possible states it can transition into

    placed: [
        //so for placed the order can go to accept or reject or cancelled
        "accepted",
        "rejected",
        "cancelled"
    ],
    accepted: [
        //cancel can only happen until here after that order cannot be cancelled  if rest accepts order then it can go to preparation or can be cancelled by the user
        "preparing",
        "cancelled"
    ],
    preparing: [
        //once the prep starts then there is no scope of cancel
        "ready"
    ],
...
const createOrder = async (
    ...
    for (const item of cart.items) {
        const menuItem = await MenuItem.findById(item.menuItem); //first check if that item even exists
    ...
    if (paymentMethod !== "card") {
        //if cod then just clear the cart
...
const getOrderById = async (orderId, userId) => {
    ...
    const isRestaurantOwner =
        order.restaurant && order.restaurant.owner.toString() === userId.toString();

    const isAssignedDeliveryPartner =
        order.deliveryPartner &&
        order.deliveryPartner.user.toString() === userId.toString();

    if (!isCustomer && !isRestaurantOwner && !isAssignedDeliveryPartner) {
...
const assignDeliveryPartner = async (
    //only a delivery partner can assign himself to the order every order has to be assigned a delivery partner
    orderId,
    userId
) => {
    ...
    if (existingOrder.deliveryPartner) {
        throw new Error("This order already has a delivery partner assigned.");
    }
    ...
    if (!order) {
        throw new Error("This order was just claimed by another delivery partner.");
    }
...
const DELIVERY_UPDATABLE_STATUSES = ["out_for_delivery", "delivered"];
...
const updateDeliveryStatus = async (orderId, userId, status) => {
    ...
    if (!order.deliveryPartner) {
        throw new Error("No delivery partner is assigned to this order.");
    }

    const deliveryPartner = await DeliveryPartner.findById(order.deliveryPartner);

    if (!deliveryPartner || deliveryPartner.user.toString() !== userId.toString()) {
...
const getDeliveryPartnerOrders = async (userId) => {
    const deliveryPartner = await DeliveryPartner.findOne({
        user: userId
    });
